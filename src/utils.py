...
def get_user_seqs2(data_file):
    ...
    max_item = max(item_set)
    num_users = len(lines)
    num_items = max_item + 2  ###???why number of items need plus 2????
    valid_rating_matrix = generate_rating_matrix_valid(user_seq, num_users, num_items)
    test_rating_matrix = generate_rating_matrix_test(user_seq, num_users, num_items)
    return user_seq, max_item, valid_rating_matrix, test_rating_matrix
...
def data_partition3(fname):
    ...
    return [user_train, user_valid, user_test, usernum, itemnum]
def res_func(input_):
    res_ = torch.where(input_>0.0, 1.0, input_)
    res__ = torch.where(res_<0.0, -1.0, res_)
    return res__
